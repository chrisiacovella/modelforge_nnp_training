...
if __name__ == "__main__":
    ...
    for version_select in versions:
        for force_loss in [0.001, 0.01]:
            for learning_rate in [5e-4, 1e-4]:
                for potential_seed, dataset_set in [(1234, 425), (1245, 435), (1255, 445)]:

                    # create a run_id based on the seeds, used for defining the local cache dir
                    run_id = f"{version_select}_ps{potential_seed}_ds{dataset_set}_fl{force_loss}_lr{learning_rate}"
                    ...
                    training_parameters = {
                        ...
                        "project": "tmqm_openff_no_norm_v2",
