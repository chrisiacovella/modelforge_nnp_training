    ...
    if add_quotes:
        for key, value in data.items():
            if isinstance(value, str):
                ...
                # make list into a long string
                #data[key] = "[" + ", ".join([f'{x}' for x in value]) + "]"
...
if __name__ == "__main__":
    ...
    potential_name = "aimnet2_sr"
    experiment_base_name = f"{dataset_name}"

    description = "Training of aimnet2 sr with tmqm_openff for Cu looking at hyperparameters for spin resolved charge model"
                        ...
                        for number_of_vector_features in [16]:
                            for number_of_per_atom_features in [128]:
                                for potential_seed, dataset_set in [(1234, 425), (1345, 435)]:
                                    ...
                                    if force_loss == 0:
                                        # we need to remove force_loss from the training_config str
