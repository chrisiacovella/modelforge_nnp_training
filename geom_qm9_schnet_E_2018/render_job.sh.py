...
if __name__ == "__main__":
    ...
    file_names = []
    experiments = {}
    for lr_scheduler_factor in [0.5]:
        for normalize in [True]:
            for initial_lr in [5e-4]:
                for number_of_filters in [64]:
                    for potential_seed, dataset_set in [(123, 42), (124, 43), (125, 44)]:
                        ...
                        run_id = f"ps{potential_seed}_ds{dataset_set}_lr{initial_lr}_norm{str(normalize)}_lr_factor{str(lr_scheduler_factor)}_nfilters{str(number_of_filters)}"
                        ...
                        training_parameters = {
                            ...
                            "project": "geom_qm9_schnet_E_2018_v1",
                            "group": "geom_qm_schnet_normTrue",
                            "tags": ["geom_qm9", "schnet", "energy", f"norm_{str(normalize)}", f"lr_{str(initial_lr)}", f"threshold_{str(lr_scheduler_factor)}"],
                            "notes": f"{run_id}; training of {potential_name} on {dataset_name} with energy and lr_scale factor {lr_scheduler_factor} and shared interactions false",
